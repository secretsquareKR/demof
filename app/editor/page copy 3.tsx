...
const handleDesignComplete = () => {
  ...
  try {
    ...
    // ❌ 기존 원인 코드: const fileName = `${orderData.customerName}_${Date.now()}.png`;
    
    //  [완벽한 해결책] 스토리지 파일 이름에는 한글을 절대로 섞지 않습니다.
    // 오직 순수한 숫자 타임스탬프로만 파일명을 만듭니다. (예: 1782816282176.png)
    const fileName = `${Date.now()}.png`;
    const filePath = fileName; // 폴더 없이 루트에 다이렉트로 매핑

    // Supabase Storage 업로드 실행 (이제 영어/숫자뿐이라 무조건 성공합니다)
    ...
    //  유저가 입력한 '한글 이름'은 스토리지 파일명이 아닌, 이곳 DB 컬럼에만 넣습니다.
    // Postgres DB는 UTF-8 한글을 완벽하게 지원하므로 깨지거나 튕기지 않습니다.
    const { data: dbData, error: dbError } = await supabaseClient
      .from('orders')
      .insert([
        {
          customer_name: orderData.customerName, // 여기에 "고진형" 날것 그대로 들어가도 오케이!
          contact: orderData.contact,
          board_color: orderData.boardColor,
    ...
    if (dbError) {
      await supabaseClient.storage.from('engraving-orders').remove([filePath]);
      throw new Error(`주문 정보 저장 실패: ${dbError.message}`);
    }
